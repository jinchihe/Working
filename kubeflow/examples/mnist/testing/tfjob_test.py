    ...
    for pair in self.params.split(","):
      ...
      if k == "namespace" or k == "image":
        ...
        util.run(["kustomize edit add configmap", configmap, "--from-literal=secretName=", secretName],
                 cwd=self.app_dir)  
        util.run(["kustomize edit add configmap", configmap, "--from-literal=secretMountPath=", secretMountPath],
                 cwd=self.app_dir)
      elif k == "envVariables":
        var_k, var_v = v.split("=", 1)
        util.run(["kustomize edit add configmap", configmap, "--from-literal=", var_k, "=", var_v],
                 cwd=self.app_dir)        
      else:
        util.run(["kustomize edit add configmap", configmap, "--from-literal=", k, "=", v],
                 cwd=self.app_dir) 

    # Create the TF job
    util.run(["kustomize build . |kubectl apply -f -"],
             cwd=self.app_dir)
